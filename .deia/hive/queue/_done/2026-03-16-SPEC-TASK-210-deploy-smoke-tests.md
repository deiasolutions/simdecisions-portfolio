...
test('should render canvas EGG with 5 panes', async ({ page }) => {
  ...
  // Canvas layout: palette (tree-browser), canvas, chat (text-pane), ir (terminal), properties (tree-browser)
  // Look for unique nodeIds from canvas.egg.md:
  // "canvas-palette", "canvas-editor", "canvas-chat", "canvas-ir", "canvas-properties"

  // Count visible panes (5 total)
  const panes = page.locator('.hhp-pane, [data-node-id]')
...
4. **Pane counting strategy:**
   - For canvas.egg.md: look for `.hhp-pane` or `[data-node-id]` selector
   - Count 5 panes (palette, canvas, chat, ir, properties)
   - If shell uses different selector, adjust based on actual rendered HTML
