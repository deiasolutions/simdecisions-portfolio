...
- [ ] `simdecisions/benchmark/collector.py` with `ResultsCollector` class (methods: `__init__`, `load_results`, `aggregate_by_track`, `compute_statistics`, `compare_tracks`, `generate_summary_table`, `export_aggregated_results`)
- [ ] `simdecisions/benchmark/significance.py` with `mann_whitney_u()` returning p-value (self-contained, no scipy)
- [ ] `simdecisions/benchmark/metrics.py` with `coefficient_of_variation()`, `recovery_rate()`, `extract_metric()` functions
- [ ] `.deia/benchmark/results/` directory created for result YAML storage
- [ ] `ResultsCollector.compute_statistics()` uses `simdecisions.des.statistics.RunningStats` (Welford's algorithm) -- NOT a reimplementation
- [ ] `ResultsCollector.generate_summary_table()` produces markdown table matching SPEC Section 3.4 format
- [ ] `mann_whitney_u()` returns p < 0.05 for clearly different groups and p near 1.0 for identical groups
- [ ] `compare_tracks()` computes deltas and p-values between baseline and simdecisions tracks
- [ ] Tests in `tests/simdecisions/benchmark/test_collector.py` -- 14+ tests passing
- [ ] Tests in `tests/simdecisions/benchmark/test_significance.py` -- 8+ tests passing
- [ ] Tests in `tests/simdecisions/benchmark/test_metrics.py` -- 8+ tests passing
