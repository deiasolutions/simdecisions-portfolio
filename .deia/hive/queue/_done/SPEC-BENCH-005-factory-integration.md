...
model: sonnet
role: bee
depends_on: [BENCH-001, BENCH-002]
---
...
- [ ] `simdecisions/benchmark/executor.py` with `BenchmarkTaskExecutor` class (methods: `__init__`, `execute_baseline`, `execute_simdecisions`, `write_result`)
- [ ] `hivenode/ledger/benchmark_events.py` defines 3 event types: `benchmark_task_start`, `benchmark_task_complete`, `benchmark_task_failed`
- [ ] Benchmark event types registered with existing ledger schema in `hivenode/ledger/schema.py`
- [ ] `simdecisions/benchmark/ledger_utils.py` with `query_task_cost()`, `count_model_calls()`, `extract_token_counts()` functions
- [ ] `execute_baseline()` captures CLOCK (wall time), queries COIN from ledger, computes CARBON via `carbon.compute_carbon()`, counts model_calls from ledger
- [ ] `execute_simdecisions()` includes `ir_used` field in SimResult
- [ ] `write_result()` serializes to YAML matching `RESULT_SCHEMA` at path `.deia/benchmark/results/{benchmark}/{task_id}_{track}_trial{N}.yml`
- [ ] Ledger events emitted on task start and completion (or failure)
- [ ] Tests in `tests/simdecisions/benchmark/test_executor.py` -- 16+ tests passing
- [ ] Tests in `tests/simdecisions/benchmark/test_ledger_utils.py` -- 10+ tests passing
- [ ] Tests in `tests/hivenode/ledger/test_benchmark_events.py` -- 6+ tests passing
- [ ] No file exceeds 500 lines
- [ ] No hardcoded currency values -- all measured or computed at runtime
