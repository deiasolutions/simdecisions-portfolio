...
# SPEC: AUTH-D — hivenode auth config rebrand ra96it to auth

## Priority
P1

## Objective
Rebrand hivenode auth configuration from ra96it_public_key/ra96it_public_key_path/ra96it_jwks_url to auth_public_key/auth_public_key_path/auth_jwks_url, update JWT issuer validation to accept BOTH ra96it and hodeia issuers.

## Context
Hivenode backend uses ra96it_* config field names and validates JWT issuer as ra96it only. This task renames config fields to generic auth_* names and implements dual-issuer JWT validation (accept both ra96it and hodeia) for backwards compat during transition.
...
## Deliverables
- [ ] Rename config fields: ra96it_public_key to auth_public_key, ra96it_public_key_path to auth_public_key_path, ra96it_jwks_url to auth_jwks_url
- [ ] Update verify_jwt() issuer validation: accept both ra96it and hodeia
- [ ] Update jwks_cache.py docstrings and references
- [ ] Search hivenode/ for all references to ra96it_* config fields and update
- [ ] Add tests for dual-issuer acceptance

## Acceptance Criteria
- [ ] Config fields renamed: ra96it_* to auth_* in config.py
- [ ] JWT issuer validation accepts both ra96it and hodeia (dual-issuer)
- [ ] All references to ra96it_public_key, ra96it_jwks_url updated in hivenode/
- [ ] Tests pass: dual-issuer acceptance + existing auth tests
- [ ] No hardcoded ra96it strings remain in config field names
- [ ] JWKS cache works with new config field names
