...
# SPEC-AUTH-B: localStorage Key Migration (ra96it to sd_auth)

## Objective

Rename localStorage keys from `ra96it_token`/`ra96it_user` to `sd_auth_token`/`sd_auth_user` (matches existing `sd_user_settings` convention). Add a backwards-compatible migration function that runs on init: if old keys exist and new keys do not, copy old to new and delete old. Migration must be idempotent. All references to old key names must be updated throughout the codebase.
...
- [ ] Key constants renamed from ra96it_token/ra96it_user to sd_auth_token/sd_auth_user
- [ ] Migration function added that copies old keys to new and deletes old
- [ ] Migration runs on authStore init
- [ ] All references to old key names updated
- [ ] Tests cover all 4 migration paths (old only, new only, both, neither)
...
- [ ] cd browser && npx vitest run src/primitives/auth/__tests__/authStore.test.ts -- authStore tests pass
