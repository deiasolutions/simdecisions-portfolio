...
# SPEC-BL-958: Build Monitor should use app standard MenuBar

## Objective

The Build Monitor EGG uses the old `ui` block format with `hideMenuBar: false` instead of the standard `uiConfig` block used by other apps (chat, canvas, efemera). Update build-monitor.egg.md to use the standard uiConfig pattern with `menuBar: true`, matching the pattern used by canvas2.egg.md and chat.egg.md. Also verify the Shell correctly reads this config and renders the MenuBar.

## Problem Analysis

Other EGGs use a `uiConfig` JSON block:
```json
{
  "menuBar": true,
...
Build Monitor instead uses an older `ui` block:
```json
{
  "hideMenuBar": false,
  "hideStatusBar": true,
...
This means the Shell may not recognize the config correctly and the MenuBar may not render with the proper app name.
...
- [ ] build-monitor.egg.md uses standard `uiConfig` block with menuBar: true
- [ ] Old `ui` block removed or replaced
- [ ] displayName set to "Build Monitor" in uiConfig

## Acceptance Criteria

- [ ] build-monitor.egg.md has a `uiConfig` JSON block with menuBar: true
- [ ] The old `ui` block is removed
...
- [ ] cd browser && npx vite build -- build passes

## Constraints

- No file over 500 lines
- No stubs
- Follow the exact uiConfig pattern from canvas2.egg.md
