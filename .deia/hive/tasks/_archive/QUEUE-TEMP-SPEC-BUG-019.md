...
# SPEC-BUG-019: Fix Canvas Drag Isolation (Stage Capture)

## Objective

Fix canvas drag-drop from palette being captured by the Shell's drag-and-drop system instead of being isolated to the canvas pane. The palette adapter must set a `canvasInternal: true` marker, TreeNodeRow must propagate it as a `canvas/internal` dataTransfer type, and ShellNodeRenderer must check for that marker and return early to ignore canvas-internal drags. The existing test file uses source code reading instead of runtime behavior verification and must be replaced with proper runtime tests.

### Root Cause

1. **Missing:** `canvasInternal: true` marker in `paletteAdapter.ts` (test expects it, grep finds none)
2. **Incomplete:** `stopPropagation()` exists in `CanvasApp.tsx` onDragOver/onDrop, but palette nodes don't set the isolation marker
3. **Untested:** Tests use source code reading instead of runtime behavior verification
4. The MIME type is also wrong: test expects `application/sd-node-type`, but actual code uses `application/phase-node` (paletteAdapter.ts line 66)

### Implementation Steps

**1. Update paletteAdapter.ts** -- In `itemToNode()` function, add `canvasInternal: true` to the `meta` object alongside the existing `dragMimeType` and `dragData`.

**2. Update TreeNodeRow.tsx** -- In `handleDragStart()`, after setting dragMimeType data, check for `canvasInternal` in node.meta. If true, set `e.dataTransfer.setData('canvas/internal', 'true')` and call `e.stopPropagation()`.

**3. Update ShellNodeRenderer.tsx** -- In both `onDragOver` and `onDrop` handlers, add early return if `e.dataTransfer.types.includes('canvas/internal')`.

**4. Verify CanvasApp.tsx** -- Lines 467 and 473 already include `event.stopPropagation()` in onDragOver and onDrop. No changes needed, just verify.

**5. Replace test file** -- Replace `canvasDragIsolation.test.tsx` with runtime behavior tests (not source code reading).
...
- [ ] cd browser && npx vitest run src/primitives/canvas/__tests__/canvasDragIsolation.test.tsx -- drag isolation tests pass
- [ ] cd browser && npx vitest run src/primitives/tree-browser/ -- tree-browser tests pass
- [ ] cd browser && npx vitest run src/shell/ -- shell tests pass
