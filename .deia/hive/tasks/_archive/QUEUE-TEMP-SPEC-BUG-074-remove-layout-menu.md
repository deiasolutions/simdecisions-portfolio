...
# SPEC-BUG-074: Remove Layout submenu from View menu

## Objective

Remove the Layout submenu from the View menu in MenuBar.tsx. Layout is determined by EGG config, not user menu selection. The Layout submenu and its associated `handleLayoutChange` function should be deleted entirely.
...
- [ ] Layout submenu block removed from View menu rendering
- [ ] `handleLayoutChange` function removed
- [ ] Layout-related tests removed or updated
...
- [ ] cd browser && npx vitest run src/shell/components/__tests__/MenuBar.test.tsx -- all MenuBar tests pass
