...
# SPEC-MCP-DISPATCH-TELEMETRY: Dispatch, Heartbeat, Sync Queue

## Objective

Implement dispatch_bee, heartbeat, status_report, cost_summary MCP tools and the sync queue writer.

## Context

Phase 1 of SPEC-HIVE-MCP-001-v2. dispatch_bee wraps the existing dispatch.py. Heartbeat enables opt-in bee-initiated telemetry (dispatch.py heartbeat continues as fallback). Sync queue writes state to ~/.shiftcenter/sync_queue/ for future cloud sync.
...
## Smoke Test
- [ ] cd hivenode && python -m pytest hive_mcp/tests/test_tools_dispatch.py -v — all tests pass
- [ ] cd hivenode && python -m pytest hive_mcp/tests/test_tools_telemetry.py -v — all tests pass
- [ ] cd hivenode && python -m pytest hive_mcp/tests/test_sync.py -v — all tests pass
...
- heartbeat wraps existing endpoint, does not replace dispatch.py heartbeat
